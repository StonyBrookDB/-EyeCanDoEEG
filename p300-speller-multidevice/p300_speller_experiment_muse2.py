"""
P300 SPELLER EXPERIMENT (MUSE2 HEADSET) — 6x6 Farwell-Donchin copy-spelling
(OFFLINE calibration).
...
This is the Muse2/BrainFlow twin of p300_speller_experiment_16ch.py (same
paradigm, markers, recorder, GUI) -- only the acquisition source differs.
Use this one when testing with a Muse2 headset instead of the customized
16-channel serial headset.
...
real-time pipeline is being validated"):
  ...
  * Acquisition: BrainFlow, wired to a Muse2 headset (BoardIds.MUSE_2_BOARD),
               connected over BLE by device name (see MUSE_NAME below). To
               switch to a different board, change board_id + params in
               main(); everything downstream (LSL, markers, recorder) is
               unchanged.
  * Startup  : two explicit gates so nothing streams before you're ready:
                 1) connect to the Muse -> print status -> press any key
                    (in the CONSOLE) to open the GUI.
...
Run (real experiment, GUI window):
    # Terminal 1 — start muselsl stream FIRST (owns the BLE connection):
    muselsl stream --name Muse-A2DA
    # Terminal 2 — run the experiment (reads from muselsl's LSL outlet):
...
Run (headless pipeline dry-run, no window — validates streaming+recording):
    muselsl stream --name Muse-A2DA   # terminal 1
...
def reconstruct_phrase(markers):
    ...
    per_char = {}
    for _, code, is_target, char_idx, _ in markers:
        if int(is_target) == 1:
...
# ---------------------------------------------------------------------------
# 2. Acquisition thread: muselsl LSL inlet -> P300_EEG LSL outlet.
#    muselsl stream owns the BLE connection; we just relay its samples.
# ---------------------------------------------------------------------------
class Acquisition(threading.Thread):
    def __init__(self, inlet, outlet, stop_event):
        super().__init__(daemon=True)
        self.inlet, self.outlet, self.stop_event = inlet, outlet, stop_event

    def run(self):
        while not self.stop_event.is_set():
            chunk, _ = self.inlet.pull_chunk(timeout=0.02)
            if chunk:
...
def main():
    ...
    # --- gate 1: find the muselsl LSL stream (must already be running) -------
    print("[muse] looking for muselsl EEG stream "
          "(run 'muselsl stream --name Muse-XXXX' in another terminal first) ...")
    streams = resolve_byprop("type", "EEG", timeout=10)
    muse_streams = [s for s in streams if "muse" in s.name().lower()]
    if not muse_streams:
        raise RuntimeError(
            "No Muse LSL stream found.\n"
            "Start it first:  muselsl stream --name Muse-A2DA"
        )
    muse_info = muse_streams[0]
    muse_inlet = StreamInlet(muse_info, max_buflen=600)

    fs    = muse_info.nominal_srate()
    n_eeg = muse_info.channel_count()

    # Parse channel names from the LSL stream descriptor
    eeg_names = []
    ch = muse_inlet.info().desc().child("channels").child("channel")
    for _ in range(n_eeg):
        label = ch.child_value("label")
        eeg_names.append(label if label else f"ch{len(eeg_names)}")
        ch = ch.next_sibling()

    print(f"[muse] found stream: {muse_info.name()}  "
          f"{n_eeg}ch @ {fs:.0f} Hz  channels={eeg_names}")
    ...
    # this try/finally guarantees the Muse session is always released.
    try:
        if args.headless:
            print("[muse] --headless: skipping the console/GUI key-press gates")
        else:
            print("\npress any key to open the speller GUI...")
            wait_for_keypress()

        # --- LSL outlets (registration only; no samples flow until gate 2) ---
        eeg_info = StreamInfo("P300_EEG", "EEG", n_eeg, fs, "float32", "p300-eeg")
        ...
        def start_streaming():
            state.acq = Acquisition(muse_inlet, eeg_outlet, stop_event)
            state.acq.start()
            if args.no_record:
